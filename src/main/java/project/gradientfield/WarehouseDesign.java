...
public class WarehouseDesign {
	...
	public static class GraphCreator {
		...
		static ListenableGraph<LengthData> createSimpleGraph(int version) {
			...
			if (version==1)
			{
				for(int i = 0; i < matrix.columnMap().size(); i++) {
					...
					for(int j = 0; j < matrix.rowMap().size(); ++j) {
						...
						if(j == 0 && i == 0){
							path = Lists.newArrayList(matrix.get(j, i),matrix.get(j+1, i+1));
							...
							path = Lists.newArrayList(matrix.get(j+1, i),matrix.get(j, i),matrix.get(j, i-1));
							Graphs.addBiPath(g, (Iterable)path);
							path = Lists.reverse(Lists.newArrayList(matrix.get(j-1, i),matrix.get(j, i),matrix.get(j, i+1)));
